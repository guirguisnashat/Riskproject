...
node[] back=new node[2];
    ...
    public node[] greedy(node[] nodes,ArrayList<distance> distances){
       node start=nodes[1];
         node target=nodes[0];
         node attacker=null;
         node defender=null;
         ...
         for(int i=0;i<start.getNodestoattack().size();i++){
             if(MainActivity.map.get(start.getfromNodestoattack(i)).getPlayer()== Color.RED && Integer.parseInt(MainActivity.map.get(start.getfromNodestoattack(i)).getMe().getText().toString())==Integer.parseInt(target.getMe().getText().toString())){
                 attacker=start;
        ...
        for(int i=0;i<start.getNodestoattack().size();i++){
            for(int j=0;j<distances.size();j++){
                ...
                if(distances.get(j).getTo().equalsIgnoreCase(target.getId()) && distances.get(j).getFrom().equalsIgnoreCase(choose.getId())&&choose.getPlayer()!=Color.BLUE){
                   neighborsheuristic.put(distances.get(j).getDistance(),choose);
                }
            }
        }
